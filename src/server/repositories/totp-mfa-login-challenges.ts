...
export async function getUnconsumedTotpMfaLoginChallenge(
  ...
  if (!Number.isFinite(now.getTime()))
    throw new RangeError("A valid challenge lookup time is required.");
...
export async function deleteExpiredTotpMfaLoginChallenges(
  ...
  await executor.delete(totpMfaLoginChallenges).where(lt(totpMfaLoginChallenges.expiresAt, now));
}
